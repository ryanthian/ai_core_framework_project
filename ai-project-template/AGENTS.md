...
When a task uses AI-Core, use `.ai/project.json`, `.ai/ai-core.yaml`, and `.ai/runs/<RUN-ID>/` as the canonical runtime state. Resume from persisted state and artifacts rather than conversation history. AI-Core coordinates the existing subsystems; it does not replace role contracts, memory rules, document review, domain precedence, or gate requirements.
