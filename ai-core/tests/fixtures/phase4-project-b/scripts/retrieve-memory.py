...
try:
    from domainlib import active_packs, relevant_domain_rules
except Exception:
    active_packs = None
    relevant_domain_rules = None
...
def main() -> None:
    ...
    domain_matches = []
    active_domain_rows = []
    if active_packs and relevant_domain_rules:
        active_domain_rows = active_packs(root)
        domain_matches = relevant_domain_rules(root, args.query)

    lines = ["# Memory Brief", "", "## Project Knowledge", ""]
    ...
    lines.extend(["", "## Active Domain Packs", ""])
    if active_domain_rows:
        for row in active_domain_rows:
            manifest = row["manifest"]
            mismatch = " VERSION_MISMATCH" if row.get("version_mismatch") else ""
            lines.append(f"- {manifest['id']}@{row['activation']['version']} ({manifest['status']}){mismatch}")
    else:
        lines.append("- None active.")

    lines.extend(["", "## Relevant Domain Rules", ""])
    if domain_matches:
        for score, manifest, rule in domain_matches:
            lines.extend([
                f"- {rule['id']} - {rule['title']} ({manifest['id']}@{manifest['version']}, {rule['type']}, {rule['confidence']})",
                f"  Source: DOMAIN_PACK {rule['source_ref']}",
    ...
    lines.extend(["", "## Domain Blind-Spot Patterns", ""])
    patterns = [item for item in domain_matches if item[2].get("type") == "BLIND_SPOT_PATTERN"]
    if patterns:
        for _score, manifest, rule in patterns:
            lines.append(f"- {rule['id']} ({manifest['id']}): {rule['statement']}")
    else:
        lines.append("- None found.")
    ...
    project_text = "\n".join([body_section(body, "Knowledge").lower() for _path, _meta, body in active_entries])
    for _score, manifest, rule in domain_matches:
        statement = rule.get("statement", "").lower()
        if ("recoverable" in statement or "rollback" in statement or "history retention" in statement) and "permanent deletion required" in args.query.lower():
            lines.extend([
                "DOMAIN_PROJECT_CONFLICT",
                f"- Domain: {rule['id']} {rule['title']} source=DOMAIN_PACK:{rule['source_ref']} confidence={rule['confidence']}",
        ...
        if "only csv" in project_text and "json" in statement:
            lines.extend([
                "DOMAIN_PROJECT_CONFLICT",
                f"- Domain: {rule['id']} {rule['title']}",
                "- Project: project memory constrains export format to CSV only.",
            ])
    for left_idx, left in enumerate(domain_matches):
        for right in domain_matches[left_idx + 1:]:
            _ls, lmanifest, lrule = left
            _rs, rmanifest, rrule = right
            if lmanifest["id"] == rmanifest["id"]:
                continue
            ltext = lrule.get("statement", "").lower()
            rtext = rrule.get("statement", "").lower()
            if "history retention" in ltext and "not retain history" in rtext:
                lines.extend([
                    "DOMAIN_DOMAIN_CONFLICT",
                    f"- Domain A: {lrule['id']} {lrule['title']} ({lmanifest['id']}@{lmanifest['version']})",
                    f"- Domain B: {rrule['id']} {rrule['title']} ({rmanifest['id']}@{rmanifest['version']})",
