...
def main() -> None:
    ...
    for section in extracted.get("sections", []):
        raw_section_text = section.get("text", "").strip()
        if not raw_section_text:
            continue
        parts = [part.strip(" -") for part in re.split(r"\n+", raw_section_text) if part.strip(" -")]
        for part_no, raw_text in enumerate(parts, start=1):
            ...
            for record in knowledge:
                if "only account owner may view statement" in record["text"] and "owner and delegated finance officer may view statement" in lower:
                    conflict = record["id"]
            ...
            items.append({
                ...
                "type": item_type,
                "text": text[:1000],
                "source_location": f"{section.get('location', '')}|part:{part_no}",
    ...
    if conflicts:
        impact = {
            ...
            "type": "DOCUMENT_CHANGE_IMPACT",
            "new_source": f".ai/documents/extracted/{args.document_id}.json",
            "conflicts": conflicts,
            "affected_knowledge": sorted({conflict["knowledge_id"] for conflict in conflicts}),
            "affected_requirements": [],
            "affected_plans": [],
            "affected_apis_data": [],
            "revalidation_needed": True,
            "recommended_action": "Review stronger/newer source, then deprecate/supersede affected knowledge if approved.",
        }
        write_json(root / ".ai" / "documents" / "processed" / f"{args.document_id}-change-impact.json", impact)
