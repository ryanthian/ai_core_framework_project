...
def analyze(requirement: str, memory: str, repo_context: str, mode: str) -> tuple[list[dict], list[dict], list[dict]]:
    ...
    if "permanently delete" in req_lower and "financial" in req_lower:
        issues.extend(
            [
                issue(
                    "DATA_IMPACT",
                    ...
                    "Financial records often require auditability, retention, reconciliation, and recovery paths.",
                    "Requirement asks to permanently delete customer financial records.",
                    question="Which authoritative retention policy permits permanent deletion?",
                    resolution="Define retention, audit, approval, backup, and rollback constraints before planning.",
                    impact="Irreversible loss of financial evidence.",
                    plan="PLAN-BS-FIN-DELETE",
                    verify="VERIFY-BS-FIN-DELETE",
                ),
                issue(
                    "COMPLIANCE",
                    "CRITICAL",
                    ...
                    "Financial records are high-risk data and cannot be removed without authority.",
                    "Requirement provides no authoritative deletion policy.",
                    question="What compliance or accounting authority governs this deletion?",
                    resolution="Obtain human/authoritative approval before any implementation.",
                    impact="Regulatory, audit, or reconciliation failure.",
                    plan="PLAN-BS-FIN-COMPLIANCE",
                    verify="VERIFY-BS-FIN-COMPLIANCE",
                ),
            ]
        )
        unknowns.append({"id": f"UNK-{len(unknowns) + 1:03d}", "statement": "Financial-record retention policy is unknown.", "why": "No authoritative policy source was provided.", "impact": "Implementation cannot safely choose hard delete semantics.", "owner": "Policy owner", "status": "OPEN"})
    ...
    if "timeout" in lower and "not specified" in lower:
        issues.append(
            issue(
                "ERROR_HANDLING",
                "HIGH",
                "OPEN",
                "API timeout behavior is not specified in document intelligence.",
                "Timeout behavior changes retry, user messaging, and reconciliation design.",
                "Document Intelligence contains timeout not specified.",
                question="What timeout, retry, and backoff behavior applies?",
                resolution="Define timeout and retry behavior before implementation planning.",
                impact="Requests may hang, duplicate, or fail without a supportable state.",
            )
        )
        unknowns.append({"id": f"UNK-{len(unknowns) + 1:03d}", "statement": "API timeout behavior is not specified.", "why": "Extracted document item is UNKNOWN.", "impact": "Implementation cannot choose retry behavior safely.", "owner": "API owner", "status": "OPEN"})

    if "idempotency" in lower and "not specified" in lower:
        issues.append(
            issue(
                "API_IMPACT",
                "HIGH",
                "OPEN",
                "API idempotency behavior is not specified in document intelligence.",
                "Retries without idempotency can duplicate operations.",
                "Document Intelligence contains idempotency not specified.",
                question="Does the API support idempotency keys or dedupe semantics?",
                resolution="Define idempotency or local dedupe before implementation planning.",
                impact="Duplicate orders or repeated side effects.",
            )
        )
        unknowns.append({"id": f"UNK-{len(unknowns) + 1:03d}", "statement": "API idempotency behavior is not specified.", "why": "Extracted document item is UNKNOWN.", "impact": "Retry design cannot be completed.", "owner": "API owner", "status": "OPEN"})
    ...
    if "AC-RULE-001" in memory or "AC-BS-001" in memory:
        if any(term in req_lower for term in ["staff", "support", "user account statements", "customer statements", "account statements"]):
            issues.append(
                issue(
                    "AUTHORIZATION",
                    "HIGH",
                    "OPEN",
                    "Domain access-control guidance requires identity, ownership, role boundary, tenant boundary, and audit checks.",
                    "Active domain pack access-control applies to staff/customer statement access.",
                    "Domain rule AC-RULE-001 / AC-BS-001 in Memory Brief.",
                    question="Which staff roles may view which statements and what audit/masking applies?",
                    resolution="Define role, ownership, tenant boundary, least privilege, and audit requirements before planning.",
                    impact="User statements may be exposed to unauthorized staff.",
                    knowledge="AC-RULE-001",
                )
            )

    if "API-RULE-001" in memory:
        if "external" in req_lower or "api" in req_lower:
            issues.append(
                issue(
                    "EXTERNAL_INTEGRATION",
                    "HIGH",
                    "OPEN",
                    "Domain external-api guidance requires timeout, retry, idempotency, authentication, rate-limit, and reconciliation behavior.",
                    "Active external-api pack applies to outbound API send.",
                    "Domain rule API-RULE-001 in Memory Brief.",
                    question="What timeout, retry, idempotency, auth, rate-limit, and reconciliation rules apply?",
                    resolution="Define external API failure semantics before implementation.",
                    impact="Duplicate sends, lost updates, or unsupported failure recovery.",
                    knowledge="API-RULE-001",
                )
            )

    if "FIN-RULE-001" in memory:
        if any(term in req_lower for term in ["expense", "profit", "8%", "calculate", "financial"]):
            issues.append(
                issue(
                    "DATA_IMPACT",
                    "MEDIUM",
                    "OPEN",
                    "Domain financial-calculation guidance requires explicit rounding, precision, negative-value, and reconciliation rules.",
                    "Financial calculation pack applies to expense/profit calculation.",
                    "Domain rule FIN-RULE-001 in Memory Brief.",
                    question="What precision and rounding point should be used?",
                    resolution="Define calculation order, decimal precision, rounding, negative values, and reconciliation tests.",
                    impact="Financial totals may be inconsistent or unreconcilable.",
                    knowledge="FIN-RULE-001",
                )
            )

    if "BILL-RULE-001" in memory:
        if any(term in req_lower for term in ["billing", "payment", "statement", "payment history", "historical"]):
            issues.append(
                issue(
                    "DATA_IMPACT",
                    "HIGH",
                    "OPEN",
                    "Domain billing guidance requires historical statement/payment traceability.",
                    "Billing pack applies to historical account statements and payment history.",
                    "Domain rule BILL-RULE-001 in Memory Brief.",
                    question="How are historical periods, adjustments, backdating, reconciliation, and audit represented?",
                    resolution="Define historical billing traceability before implementation.",
                    impact="Billing history may become inconsistent or unauditable.",
                    knowledge="BILL-RULE-001",
                )
            )

    if "support agents" in req_lower and "financial transactions" in req_lower:
        if "only owners" in memory.lower() or "private financial transactions" in memory.lower():
            issues.extend(
                [
                    issue(
                        "AUTHORIZATION",
                        ...
                        "Memory says private financial transactions are owner-scoped unless an approved exception exists.",
                        "Relevant memory contains owner-only financial transaction rule.",
                        question="Who has authority to approve support-wide access?",
                        resolution="Obtain explicit human/authoritative approval and define permission boundaries before planning.",
                        impact="Private financial data could be exposed across users.",
                        knowledge="KNOW-501",
                    ),
                    issue(
                        "PRIVACY",
                        ...
                        "Financial transaction visibility is sensitive and needs policy backing.",
                        "REQ-504 asks for every user's financial transactions.",
                        question="What privacy policy permits this access?",
                        resolution="Define least-privilege access, audit, and masking rules before implementation.",
                        impact="Privacy breach and compliance exposure.",
                        knowledge="KNOW-501",
                    ),
                ]
            )
