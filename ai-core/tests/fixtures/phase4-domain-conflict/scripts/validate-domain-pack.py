...
def validate_pack(root: Path, pack_id: str) -> tuple[int, int, int]:
    ...
    if versions_dir.exists():
        for version_dir in sorted(path for path in versions_dir.iterdir() if path.is_dir()):
            v_manifest = version_dir / "manifest.json"
            v_rules = version_dir / "rules.json"
            if not v_manifest.exists() or not v_rules.exists():
                print(f"FAIL {pack_id} version {version_dir.name} missing manifest/rules")
                fails += 1
                continue
            vdata = load_json(v_manifest)
            if vdata.get("version") != version_dir.name:
                print(f"FAIL {pack_id} version folder mismatch {version_dir.name}")
                fails += 1
            else:
                print(f"PASS version manifest {pack_id}@{version_dir.name}")
                passes += 1
            for item in load_json(v_rules).get("items", []):
                if item.get("version") != version_dir.name:
                    print(f"FAIL {item.get('id')} version mismatch")
                    fails += 1
                else:
                    print(f"PASS version rule {item.get('id')}@{version_dir.name}")
                    passes += 1
