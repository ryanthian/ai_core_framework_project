...
Active domain packs may be provided to roles as context, but roles must keep domain guidance separate from project facts.

## Default Sequence

For document input, optionally run:
...
For structured requirements, start at:
...
## Domain Context By Role

- REQUIREMENT_ANALYST receives active domain summary when relevant.
- BLIND_SPOT_REVIEWER receives domain blind-spot patterns.
- PLANNER receives domain constraints and architecture patterns.
- TESTER receives domain test patterns.
- REVIEWER checks applicable domain rules.
- VERIFIER checks applicable domain constraints.
- MEMORY_CURATOR distinguishes reusable domain candidates from project-specific knowledge.
...
- promotion of ambiguous or sensitive extracted document content
